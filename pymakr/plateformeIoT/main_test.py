...
def loraWAN():
    ...
    while 1 :
        ...
        sendData = bytes("2", "utf-8") + bytes(str(time.time()), "utf-8") + bytes("2"+str(time.time()) + ",43.59, 1.41, 245.3, 70.5, 0.1, 0.2, 0.3, 1005.22, 0.4, 0.5, 0.6, 90.1, 91.2, 15.7, 45.0, 21.3".strip(), "utf-8")
        s.send(sendData)
        time.sleep(10)
        
        print("sending objects bytes")
        # msg type                      #timestamp,                                 #latitude, #longitude, #object label
